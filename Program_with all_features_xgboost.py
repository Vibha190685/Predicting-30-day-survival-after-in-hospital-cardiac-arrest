...
params = best_params_xgboost_obj1 # Replace with your actual Optuna study object
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), 1):
    ...
    with open(f'xgboost_fold_WI_O1_{fold}.pkl', 'wb') as f:
        pickle.dump(xgb_model, f)
    
    # Obtain predicted probabilities for the positive class
    y_pred_prob = xgb_model.predict_proba(X_valid_fold)[:, 1]
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), 1):
    ...
    xgb_model.fit(X_train_fold_imputed, y_train_fold, 
                  eval_set=[(X_valid_fold_imputed, y_valid_fold)],
                  early_stopping_rounds=20, 
                  verbose=False)
    
    # Save the model for later evaluation at different thresholds (if needed)
    with open(f'xgboost_fold_Imput_mean_O1_{fold}.pkl', 'wb') as f:
        pickle.dump(xgb_model, f)
    
    # Obtain predicted probabilities for the positive class
    y_pred_prob = xgb_model.predict_proba(X_valid_fold_imputed)[:, 1]
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), 1):
    ...
    # Save the model
    with open(f'xgboost_fold_MICE_O1_{fold}.pkl', 'wb') as f:
        pickle.dump(xgb_model, f)
